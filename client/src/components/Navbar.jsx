  ...
  return (
    <>
      <nav className="fixed top-0 w-full z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md shadow-sm border-b border-outline-variant/10">
        <div className="flex flex-col lg:flex-row justify-between items-center px-4 md:px-8 py-2 md:py-3 max-w-full mx-auto">
          <div className="flex w-full lg:w-auto justify-between items-center">
            ...
            <div className="lg:hidden flex items-center">
                <button 
                  ...
                  className="text-slate-600 focus:outline-none p-1.5 bg-slate-100 rounded-md"
                >
          ...
          <div className="flex lg:hidden w-full items-center justify-between gap-1 mt-3 mb-1">
             <a href="tel:03323804080" className="px-2 py-1.5 text-xs text-primary font-bold bg-primary/10 rounded-md flex-1 text-center whitespace-nowrap">03323804080</a>
             {isUserAuthenticated ? (
                 <button onClick={() => dispatch(logout())} className="px-2 py-1.5 text-xs text-red-500 font-bold bg-red-50 rounded-md">Logout</button>
             ) : (
                 <button onClick={() => setIsAuthModalOpen(true)} className="px-4 py-1.5 text-xs bg-primary text-white font-bold rounded-md shadow-sm">Login</button>
             )}
             <a href="https://wa.me/03211660362" className="px-4 py-1.5 bg-[#25D366] text-white text-xs font-bold rounded-md shadow-sm">WhatsApp</a>
          </div>
