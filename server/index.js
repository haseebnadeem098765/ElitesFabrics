...
async function initializeDatabase() {
  ...
  // Ensure default content exists (auto-seed)
  try {
    const contentCount = await Content.countDocuments();
    if (contentCount === 0) {
      console.log('Content database is empty. Auto-seeding default content...');
      await Content.insertMany(defaultContent);
      console.log('Successfully seeded default content into MongoDB!');
    }
  } catch (err) {
...
mongoose.connect(MONGODB_URI)
  ...
  .catch(async (err) => {
    ...
    const localUri = 'mongodb://127.0.0.1:27017/elites-fabrics';
